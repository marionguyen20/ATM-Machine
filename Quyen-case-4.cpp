...
//Get some information of user
	...
	printf("\n\n\tHow much money you want to add?  "); scanf("%d",&money);
	fclose(f);
...
// Update money after adding	
	...
	fup=fopen(fileUpdate,"a+");
	fprintf(fup,"%d %s %d %d\n",s1.idnum,s1.name,s1.pass,s1.bal);
	printf("Your information now:\nName:%s\tID Number:%d\tBalance:%d",s1.name,s1.idnum,s1.bal);
	fclose(fup);}}
	fclose(f);
	
//Get information of others to update-file
	f=fopen(filePath,"r");
	while(!feof(f)){
		doc(f);
		if (idinputuser!=s1.idnum)
...
//Scan information from update-file and paste to main file
	fup=fopen(fileUpdate,"r");
	int ss=0;
		while (!feof(fup)){
			fscanf(fup,"%d",&s1.idnum);
...
//Delete information in update file	
	fup=fopen(fileUpdate,"w");
	fprintf(fup,NULL);
	fclose(fup);	
}}
